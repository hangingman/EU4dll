...
DllError InputProc9Injector(RunOptions options)
{
    DllError e;
    string pattern;
    size_t address;
    ...
    case EU4Ver.v1_29_4_0:
        ...
        if (BytePattern.tempInstance().hasSize(5, "メッセージ入力欄の取得をフック(9個目)"))
        {
            address = BytePattern.tempInstance().get(4).address; // 修正

            InputProc9CallAddress = cast(size_t)&cursor;

            // mov     r9, [rsp+50h+var_30]
            InputProc9ReturnAddress = address + 0xE;

            PatchManager.instance().addPatch(cast(void*) address, makeJmp(cast(void*) address, cast(
                    void*) InputProc9));
            writeln("JMP for Input9Injector created.");
        }
        else
        {
            e.unmatchdInputProc9Injector = true;
        }
        break;
    ...
    case EU4Ver.v1_30_5_0:
        ...
        if (BytePattern.tempInstance().hasSize(5, "メッセージ入力欄の取得をフック(9個目)"))
        {
            address = BytePattern.tempInstance().get(4).address;
            ...
            PatchManager.instance().addPatch(cast(void*) address, makeJmp(cast(void*) address, cast(
                    void*) InputProc9V130));
            writeln("JMP for Input9Injector (v1_30_plus) created.");
        }
        else
        {
            e.unmatchdInputProc9Injector = true;
    ...
    case EU4Ver.v1_31_6_0:
        ...
        if (BytePattern.tempInstance().hasSize(5, "メッセージ入力欄の取得をフック(9個目)"))
        {
            ...
            PatchManager.instance().addPatch(cast(void*) address, makeJmp(cast(void*) address, cast(
                    void*) InputProc9V131));
            writeln("JMP for Input9Injector (v1_31_plus) created.");
        }
        else
        {
            e.unmatchdInputProc9Injector = true;
    ...
    case EU4Ver.v1_33_3_0:
        ...
        if (BytePattern.tempInstance().hasSize(5, "メッセージ入力欄の取得をフック(9個目)"))
        {
            ...
            PatchManager.instance().addPatch(cast(void*) address, makeJmp(cast(void*) address, cast(
                    void*) InputProc9V132));
            writeln("JMP for Input9Injector (v1_32_plus) created.");
        }
        else
        {
            e.unmatchdInputProc9Injector = true;
        ...
        break;
    default:
        e.versionInputProc9Injector = true;
    }
