...
DllError InputProc8Injector(RunOptions options)
{
    DllError e;
    string pattern;
    size_t address;
    ...
    case EU4Ver.v1_29_4_0:
        ...
        if (BytePattern.tempInstance().hasSize(4, "メッセージ入力欄の取得をフック(8個目)"))
        {
            address = BytePattern.tempInstance().get(3).address; // 修正

            InputProc8CallAddress = cast(size_t)&cursor;

            // mov     r9, [rsp+50h+var_30]
            InputProc8ReturnAddress = address + 0xE;

            PatchManager.instance().addPatch(cast(void*) address, makeJmp(cast(void*) address, cast(
                    void*) InputProc8));
            writeln("JMP for Input8Injector created.");
        }
        else
        {
            e.unmatchdInputProc8Injector = true;
        }
        break;
    ...
    case EU4Ver.v1_30_5_0:
        ...
        if (BytePattern.tempInstance().hasSize(4, "メッセージ入力欄の取得をフック(8個目)"))
        {
            ...
            PatchManager.instance().addPatch(cast(void*) address, makeJmp(cast(void*) address, cast(
                    void*) InputProc8V130));
            writeln("JMP for Input8Injector (v1_30_plus) created.");
        }
        else
        {
            e.unmatchdInputProc8Injector = true;
    ...
    case EU4Ver.v1_31_6_0:
        ...
        if (BytePattern.tempInstance().hasSize(4, "メッセージ入力欄の取得をフック(8個目)"))
        {
            address = BytePattern.tempInstance().get(3).address;
            ...
            PatchManager.instance().addPatch(cast(void*) address, makeJmp(cast(void*) address, cast(
                    void*) InputProc8V131));
            writeln("JMP for Input8Injector (v1_31_plus) created.");
        }
        else
        {
            e.unmatchdInputProc8Injector = true;
    ...
    case EU4Ver.v1_33_3_0:
        ...
        if (BytePattern.tempInstance().hasSize(4, "メッセージ入力欄の取得をフック(8個目)"))
        {
            ...
            PatchManager.instance().addPatch(cast(void*) address, makeJmp(cast(void*) address, cast(
                    void*) InputProc8V132));
            writeln("JMP for Input8Injector (v1_32_plus) created.");
        }
        else
        {
            e.unmatchdInputProc8Injector = true;
        ...
        break;
    default:
        e.versionInputProc8Injector = true;
    }
