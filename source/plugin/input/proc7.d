...
DllError InputProc7Injector(RunOptions options)
{
    DllError e;
    string pattern;
    size_t address;
    ...
    case EU4Ver.v1_29_4_0:
        ...
        if (BytePattern.tempInstance().hasSize(3, "メッセージ入力欄の取得をフック(7個目)"))
        {
            address = BytePattern.tempInstance().get(2).address; // 修正

            InputProc7CallAddress = cast(size_t)&cursor;

            // mov     r9, [rsp+50h+var_30]
            InputProc7ReturnAddress = address + 0xE;

            PatchManager.instance().addPatch(cast(void*) address, makeJmp(cast(void*) address, cast(
                    void*) InputProc7));
            writeln("JMP for Input7Injector created.");
        }
        else
        {
            e.unmatchdInputProc7Injector = true;
        }
        break;
    ...
    case EU4Ver.v1_30_5_0:
        ...
        if (BytePattern.tempInstance().hasSize(3, "メッセージ入力欄の取得をフック(7個目)"))
        {
            ...
            PatchManager.instance().addPatch(cast(void*) address, makeJmp(cast(void*) address, cast(
                    void*) InputProc7V130));
            writeln("JMP for Input7Injector (v1_30_plus) created.");
        }
        else
        {
            e.unmatchdInputProc7Injector = true;
    ...
    case EU4Ver.v1_31_6_0:
        ...
        if (BytePattern.tempInstance().hasSize(3, "メッセージ入力欄の取得をフック(7個目)"))
        {
            ...
            PatchManager.instance().addPatch(cast(void*) address, makeJmp(cast(void*) address, cast(
                    void*) InputProc7V131));
            writeln("JMP for Input7Injector (v1_31_plus) created.");
        }
        else
        {
            e.unmatchdInputProc7Injector = true;
    ...
    case EU4Ver.v1_33_3_0:
        ...
        if (BytePattern.tempInstance().hasSize(3, "メッセージ入力欄の取得をフック(7個目)"))
        {
            ...
            PatchManager.instance().addPatch(cast(void*) address, makeJmp(cast(void*) address, cast(
                    void*) InputProc7V132));
            writeln("JMP for Input7Injector (v1_32_plus) created.");
        }
        else
        {
            e.unmatchdInputProc7Injector = true;
        ...
        break;
    default:
        e.versionInputProc7Injector = true;
    }
