...
{ // 戻り値をDllErrorからboolに変更 (役割に合わせて関数名を変更)
    ...
    log(LogLevel.info, format("PluginVersion.init: EU4 Version detected: %s", std.conv.to!string(
            options.eu4Version)));
    ...
    case EU4Ver.v1_37_5: // v1.37.5を追加
        ...
        if (BytePattern.tempInstance().hasSize(1, "EU4 Version String Pattern"))
        {
            ...
            log(LogLevel.info, "PluginVersion.init: Before logging EU4 Version String address.");
            log(LogLevel.info, format("PluginVersion.init: EU4 Version String found at 0x%x", address));
            log(LogLevel.info, "PluginVersion.init: After logging EU4 Version String address.");

            // 翻訳マップがロードされていることを確認 (最初のパッチ適用ステップ)
            log(LogLevel.info, format("PluginVersion.init: Translation map contains %d entries.", translationMap
                    .length));

            // バージョン検出のみを行い、パッチ適用は行わない（C++版の挙動に合わせる）
            log(LogLevel.info, "PluginVersion.init: EU4 Version String pattern found, but no patch applied (C++ behavior).");
            return true; // 検出成功
        }
        else
        {
            log(LogLevel.error, "PluginVersion.init: EU4 Version String pattern not found.");
            return false; // 失敗
        }
    default:
        log(LogLevel.error, format("PluginVersion.init: Unknown EU4 Version: %s", std
